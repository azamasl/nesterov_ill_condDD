...
/* Subroutine */ int active(integer *n, dd_real *l, dd_real *u, 
	integer *nbd, dd_real *x, integer *iwhere, integer *iprint, 
...
/*           otherwise set x(i)=mid(x(i), u(i), l(i)). */
	    ...
	    *cnstnd = TRUE_;
	    if (nbd[i] == 2 && u[i] - l[i] <= dd_real("0.0")) {
...
/* Subroutine */ int bmv(integer *m, dd_real *sy, dd_real *wt, integer 
	*col, dd_real *v, dd_real *p, integer *info)
{
    /* System generated locals */
...
/*     sy is a dd_real precision array of dimension m x m. */
/*       On entry sy specifies the matrix S'Y. */
/*       On exit sy is unchanged. */

/*     wt is a dd_real precision array of dimension m x m. */
...
/*     v is a dd_real precision array of dimension 2col. */
/*       On entry v specifies vector v. */
/*       On exit v is unchanged. */

/*     p is a dd_real precision array of dimension 2col. */
...
/* ======================== The end of bmv =============================== */
/* Subroutine */ int cauchy(integer *n, dd_real *x, dd_real *l, 
	dd_real *u, integer *nbd, dd_real *g, integer *iorder, integer *
	iwhere, dd_real *t, dd_real *d__, dd_real *xcp, integer *m, 
	dd_real *wy, dd_real *ws, dd_real *sy, dd_real *wt, 
	dd_real *theta, integer *col, integer *head, dd_real *p, 
	dd_real *c__, dd_real *wbp, dd_real *v, integer *nseg, 
	integer *iprint, dd_real *sbgnrm, integer *info, dd_real *
	epsmch)
{
    ...
    static dd_real f1, f2, dt, tj, tl, tu, tj0;
    static integer ibp;
    static dd_real dtm;
    extern /* Subroutine */ int bmv(integer *, dd_real *, dd_real *, 
	    integer *, dd_real *, dd_real *, integer *);
    static dd_real wmc, wmp, wmw;
    static integer col2;
    static dd_real dibp;
    static integer iter;
    static dd_real zibp, tsum, dibp2;
    static logical bnded;
    static dd_real neggi;
    static integer nfree;
    static dd_real bkmin;
    static integer nleft;
    static dd_real f2_org__;
    static integer nbreak, ibkmin;
    extern /* Subroutine */ int hpsolb(integer *, dd_real *, integer *, 
	    integer *);
...
/*     x is a dd_real precision array of dimension n. */
/*       On entry x is the starting point for the GCP computation. */
/*       On exit x is unchanged. */

/*     l is a dd_real precision array of dimension n. */
/*       On entry l is the lower bound of x. */
/*       On exit l is unchanged. */

/*     u is a dd_real precision array of dimension n. */
...
/*     g is a dd_real precision array of dimension n. */
...
/*     t is a dd_real precision working array of dimension n. */
/*       t will be used to store the break points. */

/*     d is a dd_real precision array of dimension n used to store */
/*       the Cauchy direction P(x-tg)-x. */

/*     xcp is a dd_real precision array of dimension n used to return the */
/*       GCP on exit. */
...
/*     ws, wy, sy, and wt are dd_real precision arrays. */
...
/*     theta is a dd_real precision variable. */
...
/*     p is a dd_real precision working array of dimension 2m. */
/*       p will be used to store the vector p = W^(T)d. */

/*     c is a dd_real precision working array of dimension 2m. */
/*       c will be used to store the vector c = W^(T)(xcp-x). */

/*     wbp is a dd_real precision working array of dimension 2m. */
/*       wbp will be used to store the row of W corresponding */
/*         to a breakpoint. */

/*     v is a dd_real precision working array of dimension 2m. */

/*     nseg is an integer variable. */
/*       On exit nseg records the number of quadratic segments explored */
/*         in searching for the GCP. */

/*     sg and yg are dd_real precision arrays of dimension m. */
...
/*     sbgnrm is a dd_real precision variable. */
...
/*       the derivative f1 and the vector p = W'd (for theta = 1). */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
        ...
        if (iwhere[i__] != 3 && iwhere[i__] != -1) {
            ...
            xlower = nbd[i__] <= 2 && tl <= dd_real("0.0");
            xupper = nbd[i__] >= 2 && tu <= dd_real("0.0");
            /*              reset iwhere(i). */
            iwhere[i__] = 0;
            if (xlower) {
                if (neggi <= dd_real("0.0")) {
                    iwhere[i__] = 1;
                }
            } else if (xupper) {
                if (neggi >= dd_real("0.0")) {
        ...
        if (iwhere[i__] != 0 && iwhere[i__] != -1) {
            ...
            if (nbd[i__] <= 2 && nbd[i__] != 0 && neggi < dd_real("0.0")) {
            ...
            } else if (nbd[i__] >= 2 && neggi > dd_real("0.0")) {
                /*                                 x(i) + d(i) is bounded; compute t(i). */
                ++nbreak;
                iorder[nbreak] = i__;
                t[nbreak] = tu / neggi;
...
L777:
    ...
    dt = tj - tj0;
    if (dt != 0. && *iprint >= 100) {
         cout << "Piece %ld "<< *nseg << "--f1, f2 at start point "<< f1<<" " << f2<< " %.2e\n"<< endl;
    ...
    if (*col > 0) {
        ...
        f1 += dibp * wmc;
        f2 = f2 + dibp * dd_real("2.0") * wmp - dibp2 * wmw;
    }
    /* Computing MAX */
...
L888:
    if (*iprint >= 99) {
        cout << "GCP found in this segment. Piece " << *nseg << " --f1, f2 at start point " << f1 << f2 << endl;
        cout << "Distance to the stationary point = " << dtm << endl;
    }
    if (dtm <= dd_real("0.0")) {
        dtm = dd_real("0.0");
...
/* Subroutine */ int cmprlb(integer *n, integer *m, dd_real *x, 
	dd_real *g, dd_real *ws, dd_real *wy, dd_real *sy, 
	dd_real *wt, dd_real *z__, dd_real *r__, dd_real *wa, 
	integer *index, dd_real *theta, integer *col, integer *head, 
...
/* Subroutine */ int formk(integer *n, integer *nsub, integer *ind, integer *
	...
	updatd, dd_real *wn, dd_real *wn1, integer *m, dd_real *ws, 
	dd_real *wy, dd_real *sy, dd_real *theta, integer *col, 
	integer *head, integer *info)
{
    ...
    static integer upcl;
    static dd_real temp1, temp2, temp3, temp4;
...
/*     wn is a dd_real precision array of dimension 2m x 2m. */
...
/*     wn1 is a dd_real precision array of dimension 2m x 2m. */
...
/*     ws, wy, sy, and wtyy are dd_real precision arrays; */
/*     theta is a dd_real precision variable; */
/*     col is an integer variable; */
/*     head is an integer variable. */
...
/* Subroutine */ int formt(integer *m, dd_real *wt, dd_real *sy, 
	dd_real *ss, integer *col, dd_real *theta, integer *info)
{
    /* System generated locals */
...
/* Subroutine */ int hpsolb(integer *n, dd_real *t, integer *iorder, 
        integer *iheap)
{
    /* System generated locals */
    integer i__1;
...
/*     t is a dd_real precision array of dimension n. */
...
/* Subroutine */ int matupd(integer *n, integer *m, dd_real *ws, 
	dd_real *wy, dd_real *sy, dd_real *ss, dd_real *d__, 
	dd_real *r__, integer *itail, integer *iupdat, integer *col, 
	integer *head, dd_real *theta, dd_real *rr, dd_real *dr, 
	dd_real *stp, dd_real *dtd)
{
    /* System generated locals */
    integer ws_dim1, ws_offset, wy_dim1, wy_offset, sy_dim1, sy_offset, 
...
/* Subroutine */ int projgr(integer *n, dd_real *l, dd_real *u, 
        integer *nbd, dd_real *x, dd_real *g, dd_real *sbgnrm)
{
    /* System generated locals */
    integer i__1;
    dd_real d__1, d__2;
...
/* Subroutine */ int subsm(integer *n, integer *m, integer *nsub, integer *
	ind, dd_real *l, dd_real *u, integer *nbd, dd_real *x, 
	dd_real *d__, dd_real *xp, dd_real *ws, dd_real *wy, 
	dd_real *theta, dd_real *xx, dd_real *gg, integer *col, 
	integer *head, integer *iword, dd_real *wv, dd_real *wn, 
	integer *iprint, integer *info)
{
    ...
    static dd_real dd_p__, temp1, temp2, alpha;
    static integer pointr;
...
/*     d is a dd_real precision array of dimension n. */
/*       On entry d is the reduced gradient of Q at xcp. */
/*       On exit d is the Newton direction of Q. */

/*    xp is a dd_real precision array of dimension n. */
/*       used to safeguard the projected Newton direction */

/*    xx is a dd_real precision array of dimension n */
/*       On entry it holds the current iterate */
/*       On output it is unchanged */
/*    gg is a dd_real precision array of dimension n */
/*       On entry it holds the gradient at the current iterate */
/*       On output it is unchanged */

/*     ws and wy are dd_real precision arrays; */
/*     theta is a dd_real precision variable; */
/*     col is an integer variable; */
/*     head is an integer variable. */
/*       On entry they store the information defining the */
...
/*     wv is a dd_real precision working array of dimension 2m. */
...
/*     ************ */
    ...
    *iword = 0;
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
        ...
        dk = d__[i__];
        if (nbd[k] != 0) {
            if (dk < dd_real("0.0") && nbd[k] <= 2) {
                temp2 = l[k] - x[k];
                if (temp2 >= dd_real("0.0")) {
                    temp1 = dd_real("0.0");
                } else if (dk * alpha < temp2) {
                    temp1 = temp2 / dk;
                }
            } else if (dk > dd_real("0.0") && nbd[k] >= 2) {
                temp2 = u[k] - x[k];
                if (temp2 <= dd_real("0.0")) {
                    temp1 = dd_real("0.0");
                } else if (dk * alpha > temp2) {
    ...
    if (alpha < dd_real("0.0")) {
        dk = d__[ibd];
        k = ind[ibd];
        if (dk > dd_real("0.0")) {
            x[k] = u[k];
            d__[ibd] = dd_real("0.0");
        } else if (dk < dd_real("0.0")) {
            x[k] = l[k];
            d__[ibd] = dd_real("0.0");
        }
    }
